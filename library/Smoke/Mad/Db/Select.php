 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Smoke_Mad_Db_Select extends Zend_Db_Select
{
    ...
    /**
     * Horde_Db_Adapter_Abstract object.
     *
    ...
    /**
     * Adapter specific auto setting if should quote db identifies
     * @var bool
     */
    public $_autoQuoteIdentifiers = null;
     ...
     */
    public function __construct(Horde_Db_Adapter_Abstract $adapter)
    {
     ...
     * @return Smoke_Mad_Db_Select This Smoke_Mad_Db_Select object.
     */
    ...
    public function columns($cols = '*', $correlationName = null)
    {
        ...
        if (!array_key_exists($correlationName, $this->_parts[self::FROM])) {
            /**
             * @see Smoke_Mad_Db_Select_Exception
             */
            require_once 'Zend/Db/Select/Exception.php';
            throw new Smoke_Mad_Db_Select_Exception("No table has been specified for the FROM clause");
        }
     ...
     *
     * The first parameter has to be an array of Smoke_Mad_Db_Select or
     * sql query strings.
     *
     * <code>
     * $sql1 = $db->select();
    ...
    public function union($select = array(), $type = self::SQL_UNION)
    {
        if (!is_array($select)) {
            ...
            throw new Zend_Db_Select_Exception(
                "union() only accepts an array of Smoke_Mad_Db_Select instances of sql query strings."
            );
        }
    ...
    {
        if ($value !== null) {
            $cond = $this->_adapterQuoteInto($cond, $value, $type);
        }
     ...
     * @return mysqli_result|mixed @see Horde_Db_Adapter_Abstract::execute()
     */
    public function query($fetchMode = null, $bind = array())
    {
        ...
        return $this->_adapter->execute((string) $this);
    }
    ...
    /**
     * Gets the Horde_Db_Adapter_Abstract for this
     * particular Smoke_Mad_Db_Select object.
     *
    ...
    public function _joinUsing($type, $name, $cond, $cols = '*', $schema = null)
    {
        ...
        $join  = $this->_adapterQuoteIdentifier(key($this->_parts[self::FROM]), true);
        $from  = $this->_adapterQuoteIdentifier($this->_uniqueCorrelation($name), true);
    ...
    protected function _where($condition, $value = null, $type = null, $bool = true)
    {
        ...
        if ($value !== null) {
            $condition = $this->_adapterQuoteInto($condition, $value, $type);
        }
    ...
    protected function _getQuotedSchema($schema = null)
    {
        ...
        return $this->_adapterQuoteIdentifier($schema, true) . '.';
    }
    ...
    {
        return $this->_adapterQuoteTableAs($tableName, $correlationName, true);
    }
    ...
    protected function _renderColumns($sql)
    {
        ...
        foreach ($this->_parts[self::COLUMNS] as $columnEntry) {
            ...
            if ($column instanceof Zend_Db_Expr) {
                $columns[] = $this->_adapterQuoteColumnAs($column, $alias, true);
            } else {
                ...
                if (empty($correlationName)) {
                    ...
                    $columns[] = $this->_adapterQuoteColumnAs(array($correlationName, $column), $alias, true);
                }
            }
        }
    ...
    protected function _renderGroup($sql)
    {
        if ($this->_parts[self::FROM] && $this->_parts[self::GROUP]) {
            ...
            foreach ($this->_parts[self::GROUP] as $term) {
                $group[] = $this->_adapterQuoteIdentifier($term, true);
            }
            ...
            foreach ($this->_parts[self::ORDER] as $term) {
                if (is_array($term)) {
                    if(is_numeric($term[0]) && strval(intval($term[0])) == $term[0]) {
                        ...
                        $order[] = $this->_adapterQuoteIdentifier($term[0], true) . ' ' . $term[1];
                    }
                } else if (is_numeric($term) && strval(intval($term)) == $term) {
                    $order[] = (int)trim($term);
                } else {
                    $order[] = $this->_adapterQuoteIdentifier($term, true);
                }
            }
    ...
    protected function _renderLimitoffset($sql)
    {
         ...
         */
        if ($count > 0) {
            $sql = trim($this->_adapterLimit($sql, $count, $offset));
        }
    ...
    // Methods to adapt implementations
     ...
     */
    public function _adapterQuoteColumnAs($ident, $alias, $auto=false)
    {
        return $this->_adapterQuoteIdentifierAs($ident, $alias, $auto);
    }
     ...
     * @return string The quoted identifier and alias.
     */
    public function _adapterQuoteTableAs($ident, $alias = null, $auto = false)
    {
        return $this->_adapterQuoteIdentifierAs($ident, $alias, $auto);
    }    
    /**
     ...
     * @return string The quoted identifier and alias.
     */
    protected function _adapterQuoteIdentifierAs($ident, $alias = null, $auto = false, $as = ' AS ')
    {
            ...
            if (is_array($ident)) {
                ...
                foreach ($ident as $segment) {
                    if ($segment instanceof Zend_Db_Expr) {
                        ...
                        $segments[] = $this->_adapterQuoteIdentifier($segment, $auto);
                    }
                }
                ...
                $quoted = $this->_adapterQuoteIdentifier($ident, $auto);
            }
        }
        if ($alias !== null) {
            $quoted .= $as . $this->_adapterQuoteIdentifier($alias, $auto);
        }
        return $quoted;
    }
     ...
     * @return string        The quoted identifier and alias.
     */
    protected function _adapterQuoteIdentifier($value, $auto=false)
    {
     ...
     * @return string
     */
     public function _adapterLimit($sql, $count, $offset = 0)
     {
        $count = intval($count);
        if ($count <= 0) {
     ...
     */
    public function _adapterQuoteInto($text, $value, $type = null, $count = null)
    {
        if ($count === null) {
            return str_replace('?', $this->_adapter->quote($value), $text);
        } else {
            while ($count > 0) {
                if (strpos($text, '?') !== false) {
                    $text = substr_replace($text, $this->_adapter->quote($value), strpos($text, '?'), 1);
                }
                --$count;
            }
            return $text;
